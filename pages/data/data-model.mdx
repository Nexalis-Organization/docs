...
| `gold_<user_name>_hf_std`  | High frequency (per‑second raw data)        | None (raw values)                   | Yes           | Alarms, troubleshooting, detailed monitoring |
...
- **assetType** — *Presence:* `both`; standardized device type (e.g., inverter, meter)  
- **logicalNode** — *Presence:* `both`; standardized function type (e.g., DC Measurements)  
- **dataObject** — *Presence:* `both`; standardized tag (e.g., phase voltage)  
- **subDataObject** — *Presence:* `both`; finer breakdown (e.g., phase A)  
- **engUnits** — *Presence:* `both`; standardized engineering units  
- **measurementType** — *Presence:* `both`; Analog vs Discrete  
...
- **triggerType** — *Presence:* `both`; agent configuration for when to send data  

...
<Note>
This tutorial uses the **Compulab IOT-GATE-IMX8PLUS** as the target hardware in its examples, but Nexalis is compatible with many more hardware models — see [Supported hardware](#supported-hardware).
</Note>
...
The target hardware is connected to the onboarding device with a **micro-USB cable**; your phone talks to the onboarding device over **Bluetooth**. For the onboarding process, on the **IOT-GATE-IMX8PLUS**, the micro-USB cable must be plugged into the **PROG** port — **not the Console port**. Once flashing succeeds and the target is power-cycled, the onboarding device and the phone are no longer needed.

---

## Supported hardware

The Nexalis Agent runs on standard x86 and ARM industrial edge hardware, so in many cases you can reuse hardware you already have — there's no need to buy a brand-new device for every deployment. When you **do** need to provision new hardware, the model we recommend — and the one used as the example throughout this guide — is the **Compulab IOT-GATE-IMX8PLUS** (PN `IOTG-IMX8PLUS-C1800QM-D4-N32-WB-TIC`):

- Based on the NXP **i.MX 8M Plus** (ARM), fanless and DIN-rail mountable.
- 4 GB RAM / 32 GB eMMC, Wi-Fi + Ethernet — comfortably above the Nexalis Agent baseline.
- **Around \$500 per device**, which makes it a cost-effective default for most sites.
- Designed for **24/7 continuous industrial operation** — MTBF over **22 years**, with a **5-year warranty**.
- **Passive (fanless) cooling**, which significantly improves reliability.
- Industrial temperature option (**TIC**): certified for **-40 °C to +80 °C** operation.

Full specifications are available on the [Compulab product page](https://www.compulab.com/products/iot-gateways/iot-gate-imx8plus-industrial-arm-iot-gateway/).
...
- A **Nexalis Flash account** — or create one directly in the app (steps 1–3).
...
<Warning>
**Do not connect any other cables to the target hardware during flashing.** Only the micro-USB cable to the onboarding device (and power) may be connected. Any other cable — for example a **USB-to-CANbus converter** — can make the flashing process fail. Connect such cables only **after the flash has completed** and the device has been power cycled (step 11).
</Warning>

### 1. Log in (or sign up)

Open the app to reach the landing page. Sign in with your Nexalis Flash account email and password. **Don't have an account yet?** Tap **Sign up** to create one (steps 2–3), then come back here to log in.
...
<Warning>
On the **IOT-GATE-IMX8PLUS**, plug the micro-USB cable into the **PROG** port of the target hardware — **not the Console port**. Flashing only works through the PROG port.
</Warning>

{/* TODO: add front-panel photo of the IOT-GATE-IMX8PLUS highlighting the PROG port, e.g. /images/nexalis-agent-onboarding-new-hardware-deployment/iot-gate-imx8plus-front.jpg */}
...
After the power cycle, the target boots the Nexalis Agent image, connects to the site network, registers in **Nexalis Edge Manager** under the name you chose, and begins streaming data to **Nexalis Cloud**. At this point you can connect any other cables the device needs (for example a USB-to-CANbus converter).

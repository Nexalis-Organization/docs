...
{{
  'token' '{READ_TOKEN}'
  'class' 'nx.value'
  'labels' {{ 'siteName' 'TX01' 'assetType' 'INV' 'dataObject' 'TotW' }}
...
> 💡 **Tip:** Learn how to use filters and regular expressions in the `labels` parameter to target specific data points. See [Filtering Data](./real-time-api#filtering-data) for examples.
...
# API query using the trapezoidal averages macro
query = f"""
{{
  'token' '{READ_TOKEN}'
  'labels' {{ 'siteName' 'TX01' 'assetType' 'INV' 'dataObject' 'TotW' }}
...
- Explore the [PowerBI Connector](./powerbi-connector) for dashboards

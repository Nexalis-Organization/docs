...
The default plugins directory is `/var/lib/grafana/plugins`. To confirm the path your server actually uses, ask Grafana — it logs every path at startup:
...
To update, replace the folder with the newer release and restart Grafana:

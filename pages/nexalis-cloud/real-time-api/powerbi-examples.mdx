...
Connect Power BI to Nexalis Cloud's real-time API to query metadata and create interactive dashboards. This guide shows the recommended approach using Power BI's Web connector with parameters.
...
### Step 3: Create the Metadata Query
...
1. Click **Invoke**, which will take you to the warning message "Please specify how to connect".
...
Click **Close & Apply** to load the metadata table into Power BI.
...
1. **Start with Metadata First**: It's more efficient to query and explore the list of available tags before fetching tags with their values. The metadata query is lightweight and fast, while fetching actual time-series values transfers much more data and takes significantly longer.
...
2. **Parameter-Based Configuration**: Parametrizing the URL and token allows you to:
   ...
   - **Switch environments easily**: Change between dev, staging, and production by updating parameters once

3. **Complete Metadata**: Get all labels and attributes in one query, including descriptions, units, multipliers, and adders

4. **Scalable**: Works for any number of sites and devices without modifying the query

5. **Self-Documenting**: Includes descriptions and engineering units for each data point
...
Once you have the metadata table loaded:

1. **Create relationships** with other tables in your data model
2. **Build visualizations** using the metadata to label and filter data
3. **Add time-series queries** to fetch actual values (see advanced examples below)
4. **Schedule refresh** to keep metadata up to date

---

## Advanced: Fetching Time-Series Data

To fetch actual time-series values, create another query using the `/exec` endpoint with WarpScript:

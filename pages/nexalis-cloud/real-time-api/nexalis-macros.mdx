...
Fetches time-series data from Nexalis Cloud and bucketizes it into fixed-width time intervals using a Warp 10 bucketizer such as `last`, `first`, or `mean`. This macro calls `@nexalis/scale` by default, so returned values are scaled unless disabled.
...
- Query multiple GTS with label or attribute filters
...
| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `token` | STRING | Yes | Warp10 read token with fetch permissions |
| `start` | STRING or LONG | Yes | Start timestamp (ISO8601 string or microseconds) |
| `end` | STRING or LONG | Yes | End timestamp (ISO8601 string or microseconds) |
| `bucket_size` | LONG | Yes | Bucket width in minutes |
| `bucketizer` | STRING | Yes | Warp 10 bucketizer to apply, for example `last`, `first`, or `mean` |
| `labels` | MAP | Yes | Labels and attributes used to filter GTS (supports regexp with `~` prefix) |
| `scaling` | BOOLEAN | No | Applies `@nexalis/scale` to the result (default: `true`) |
| `class` | STRING | No | Selector for the GTS classes (default: `"nx.value"`) |

### Returns

A list of bucketized GTS over the requested time range.
...
1. Fetches GTS matching the requested class, labels, and attributes, with one boundary value before and after the interval.
2. Normalizes timestamps and aligns them to the requested bucket size.
3. Applies the selected Warp 10 bucketizer over fixed-width buckets.
